...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, x_std, mean, var, std, gamma, eps, N, layernorm)
        ...
        # use running averages to normalize
...
def batchnorm_backward(dout, cache):
    ...
    x, x_std, mean, _, std, gamma, _, N, layernorm = cache

    dbeta = np.sum(dout, axis=layernorm)
    dgamma = np.sum(dout * x_std, axis=layernorm)
    dx_std = dout * gamma
    dstd = -1.0 / (std**2) * np.sum((x - mean) * dx_std, axis=0)
    dvar = 0.5 / std * dstd
    dxm1 = dvar / N * 2.0 * (x - mean)
    dxm2 = dx_std / std
    dxm = dxm1 + dxm2
    dmean = -1.0 * dxm
    dx1 = (dmean / N).sum(axis=0)
    dx2 = dxm
    dx = dx1 + dx2
...
def batchnorm_backward_alt(dout, cache):
    ...
    _, x_std, _, _, std, gamma, _, N, layernorm = cache

    dbeta = np.sum(dout, axis=layernorm)
    dgamma = np.sum(dout * x_std, axis=layernorm)
    dx_std = dout * gamma
    dx = (
        N * dx_std - np.sum(dx_std, axis=0) - np.sum(dx_std * x_std, axis=0) * x_std
    ) / (N * std)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # Naive loop over samples, filters, and spatial locations
    for n in range(N):
        for f in range(F):
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    # reshape to (N*H*W, C) for vanilla batchnorm
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_std = (x - mean) / std
    x_std = x_std.T.reshape(N, C, H, W)
    out = gamma * x_std + beta

    cache = (x, x_std, mean, var, std, gamma, beta, eps, size)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    _, x_std, _, _, std, gamma, _, _, size = cache

    dbeta = dout.sum(axis=(0, 2, 3), keepdims=True)
    dgamma = np.sum(dout * x_std, axis=(0, 2, 3), keepdims=True)
    x_std = x_std.reshape(size).T
    M = x_std.shape[0]
    dx_std = dout * gamma
    dx_std = dx_std.reshape(size).T
    dx = (
        dx_std - np.sum(dx_std, axis=0) / M - np.sum(dx_std * x_std, axis=0) * x_std / M
    ) / std
