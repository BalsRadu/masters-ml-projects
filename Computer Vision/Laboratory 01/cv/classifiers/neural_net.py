...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = -np.sum(np.log(scores_softmax[np.arange(N), y])) / N
        ...
        grads["W2"] = hidden.T.dot(scores_softmax)
        grads["W2"] += 2 * reg * W2
        grads["b2"] = np.sum(scores_softmax, axis=0)

        dhidden = scores_softmax.dot(W2.T)
        dhidden[(X.dot(W1) + b1) <= 0] = 0
